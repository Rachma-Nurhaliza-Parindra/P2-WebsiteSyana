		...
		<div class="content">
			...
			<form action="" method="POST">
				...
				<div>
					...
					<input type="email" name="email" class="<?= form_error('email') ? 'invalid' : '' ?>" placeholder="example@example.com" value="<?= form_error('email') ? set_value('email') : $anggota->email ?>" />
					<div class="invalid-feedback">
						<?= form_error('email') ?>
					</div>
				</div>
				...
				<div style="display: grid; gap: 5px;">
					...
					<select name="prodi" id="prodi" class="<?= form_error('prodi') ? 'invalid' : '' ?>" style="padding: 10px; border-radius: 3px;">
						...
						<?php foreach ($prodi as $p) : ?>
							<option value="<?= $p ?>" <?= $anggota->prodi == $p ? 'selected' : '' ?>><?= $p ?></option>
						<?php endforeach ?>
					</select>
				</div>
				...
				<div>
					...
					<input type="number" name="no_hp" pattern="[0-9]*" class="<?= form_error('no_hp') ? 'invalid' : '' ?>" placeholder="0xxxxxxxx" value="<?= form_error('no_hp') ? set_value('no_hp') : $anggota->no_hp ?>" />
					<div class="invalid-feedback">
						<?= form_error('no_hp') ?>
					</div>
				</div>
